(function(window, document, $, undefined) {
    ...
    $(function() {
        ...
        if ($('#c3chart_zoom_ksamobily').length) {
            var rows = ['sites'];
            var rowsdate = ['x'];
            var datevalues= [];
            httpGetAsync("http://ericssonquiz.com/dashboard-backend/get.php?country=KSA_MOBILY&type=spd", function(resp) {
                const json = resp;
                const obj = JSON.parse(json);
                for (var i = 0; i < obj.data.length; i++) {
                    // alert( obj.data[i][1]);
                    if(obj.data[i][0] != '0000-00-00'){
                        rows.push( obj.data[i][1]);
                        rowsdate.push( obj.data[i][0]);
                        datevalues.push( obj.data[i][0]);
                    }
                }
                var chart = c3.generate({
                    bindto: "",
