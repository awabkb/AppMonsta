                ...
                },
                {
                    ...
                    onCreatedCell: (td, cellData) => {
                        if (cellData) {
                            ...
                            td.innerHTML = `<span class="tooltip dotted">${integration?.integrateStart?.slice(0, 16)}
                                                <span class="message right">
                                                        <div>ASP: ${integration?.asp}</div>
                                                        <div>Integration Start Time: ${integration?.integrateStart?.slice(0, 16) || ""}</div>
                                                        <div>Integration End Time: ${integration?.integrateEnd?.slice(0, 16) || ""}</div>
                                                        <div>Duration: ${integration?.integrationTime}</div>
                                                        <div>Download Start Time: ${integration?.downloadStart?.slice(0, 16) || ""}</div>
                                                        <div>Download End Time: ${integration?.downloadEnd?.slice(0, 16) || ""}</div>
                                                        <div>Field Engineer: ${integration?.user}</div>
                                                        <div>Android Version: ${integration?.androidVersion}</div>
                                                </span>
                                             </span>`;
                        }
                    },
                ...
                },
                {
                    ...
                    onCreatedCell: (td, cellData) => {
                        if (cellData) {
                            ...
                            console.log(diagnostic?.siteVisit?.startTime?.split(0, 16));
                            td.innerHTML = `<span class="tooltip dotted">${diagnostic?.siteVisit?.startTime?.slice(0, 16)}
                                                <span class="message left">
                                                    ...
                                                    <div>Android Version: ${diagnostic?.siteVisit?.appVersion}</div>
                                                </span>
                                            </span>`;
                        }
                    },
